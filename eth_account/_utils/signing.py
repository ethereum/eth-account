...
def hash_of_signed_transaction(txn_obj: Transaction) -> bytes:
    """
...
def sign_transaction_hash(
    account: PrivateKey, transaction_hash: bytes, chain_id: Optional[int] = None
) -> Tuple[int, int, int]:
...
def _pad_to_eth_word(bytes_val: bytes) -> bytes:
...
def sign_message_hash(key: PrivateKey, msg_hash: bytes) -> Tuple[int, int, int, bytes]:
